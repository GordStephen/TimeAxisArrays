...
A2dresult = TimeAxisArray([A2d.data[[2,4,13],1] A2d.data[[2,4,11],2] A2d.data[[1,7,13],3]],
                          timestamps(A2d)[[3,10,14]], A2d.axes[2])
A2dresult.data[[1,3,5,6,8]] = NaN
@test isequal(downsample(A2d, week, last, maximum), A2dresult)
@test isequal(downsample(A2d, week, last, maximum), downsample(A2d, week, last, x->maximum(x,1), lift=false))

# moving
@test moving(A1d2, mean, 3) ≈ TimeAxisArray(map(mean, TimeAxisArray[A1d2[1:3], A1d2[2:4], A1d2[3:5]]), timestamps(A1d2)[3:5])
@test isequal(moving(A2d, x->maximum(x,1), 5, lift=false), moving(A2d, maximum, 5))
@test isequal(moving(A2d, maximum, 5),
              TimeAxisArray([[NaN A2d[4,2] NaN];
                             [A2d[2,1] A2d[4,2] NaN];
                             [A2d[3,1] NaN NaN];
                             [A2d[4,1] NaN NaN];
                             [A2d[5,1] NaN A2d[7,3]];
                             [A2d[7,1] NaN A2d[7,3]];
                             [NaN NaN A2d[7,3]];
                             [NaN A2d[11,2] A2d[12,3]];
                             [NaN NaN A2d[13,3]];
                             [NaN NaN A2d[13,3]]
                            ], timestamps(A2d)[5:end], A2d.axes[2:end]...))
